...
class ModelManager(object):
    ...
    def __init__(self, cat="cat1", use_pre=True, epoch=15, lr=0.1, wordNgrams=2, minCount=40, dim=100, loss="softmax", thread=12, neg=5):
    ...
    def get_new_model(self, output):
        """Get a new model and save it in resources/models/ with the output name.

        It first fetches the data from cassandra and write it to a temporary file, following the fastText format.
        Then it pre-processes the data (tokenization + lowercase).
        Then it trains a model using fastText and it saves it at output location.
    ...
    def _write_training_data_to_file(self):
        ...
        if self.use_pre:
            with open(resources_path + "usenet_data_{}.txt".format(self.cat),  'r') as basef:
                for line in basef:
                    f.write(line)
    ...
    def _train_tagging_model(self, output):
        ...
        Pre-processing of the input file must have been done prior to training the model.
        ...
        log.info('Model saved at {}.'.format(resources_path + "models/" + output))

    def _remove_tempfile(self):
        os.remove(self.tempfilename)
        # TODO How To ? https://docs.python.org/2/library/tempfile.html#tempfile.NamedTemporaryFile
