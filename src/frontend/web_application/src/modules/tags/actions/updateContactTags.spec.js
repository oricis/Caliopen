...
jest.mock('./updateTagCollection', () => ({
  ...
  (dispatch) => {
    ...
    dispatch({ type: 'updateTagCollection', payload: { type, contact_id: entity.contact_id, tags, lazy } });
...
describe('modules tags - actions - updateContactTags', () => {
  it('updates tag collection for each contact then invalidate', () => {
    const store = mockStore({
      contact: {
        contactsById: {
          aabbcc01: { contact_id: 'aabbcc01', tags: [{ label: 'Foo', name: 'foo' }] },
          aabbcc02: { contact_id: 'aabbcc02', tags: [{ label: 'Foo', name: 'foo' }] },
        },
      },
      tag: {
    ...
    const expectedActions = [
      { type: 'updateTagCollection', payload: { type: 'contact', contact_id: 'aabbcc01', lazy: true, tags } },
      { type: 'updateTagCollection', payload: { type: 'contact', contact_id: 'aabbcc02', lazy: true, tags } },
      { type: 'invalidate' },
    ];
    const action = updateContactTags(i18n, ['aabbcc01', 'aabbcc02'], tagCollection);
