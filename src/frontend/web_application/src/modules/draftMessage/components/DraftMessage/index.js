...
const identityStateSelector = (
  state,
  { identHoc: { identities, isFetching } }
) => ({
  identities,
  isFetching,
});
...
const parentMessageSelector = createSelector(
  ...
  ({ draftMessage }, sentMessages) =>
    draftMessage &&
    sentMessages.find((item) => item.message_id === draftMessage.parent_id)
);
...
const mapStateToProps = createSelector(
  ...
  (
    ...
    const canEditRecipients =
      messages.some((message) => !message.is_draft) === false;
    ...
    return {
      ...
      draftEncryption:
        draftMessage && messageEncryptionStatus[draftMessage.message_id],
      encryptionStatus:
        original &&
        messageEncryptionStatus[original.message_id] &&
...
const onUpdateEntityTags = (
  ...
  const savedDraft = await dispatch(
    saveDraft(
      { internalId, draft: entity, message },
      {
        withThrottle: false,
        force: true,
      }
  ...
  const messageUpTodate = await dispatch(
    updateTagCollection(i18n, { type, entity: savedDraft, tags })
  );
