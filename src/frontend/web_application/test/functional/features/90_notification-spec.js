...
describe('Notification', () => {
  ...
  describe('Timeline', () => {
    it('shows N new messages and clear', async () => {
      await browser.wait(EC.presenceOf(element(by.cssContainingText('.s-timeline__new-msg', 'new messages'))), THROTTLE_DURATION);
      expect(element(by.cssContainingText('.s-timeline__new-msg', 'You have 4 new messages')).isPresent()).toEqual(true);
      expect(element(by.css('.l-notification-center__notification-item-message')).isPresent()).toEqual(false);
      await element(by.cssContainingText('.s-timeline__new-msg', 'You have 4 new messages')).click();
      await browser.wait(EC.stalenessOf(element(by.css('.s-timeline__new-msg'))), 1000);
    });
  });
  ...
  xdescribe('Discussion', () => {
    it('shows N new messages and clear', async () => {
      ...
      expect(element(by.cssContainingText('.m-message', 'new messages for the discussion')).isPresent()).toEqual(true);
      expect(element(by.cssContainingText('.m-message', 'new other messages')).isPresent()).toEqual(false);
      expect(element(by.css('.notification')).isPresent()).toEqual(false);
      await element(by.cssContainingText('.m-message', 'new messages for the discussion')).click();
      expect(element(by.cssContainingText('.m-message', 'new messages')).isPresent()).toEqual(false);
    });
    ...
    it('shows N new messages and a specific note for other discussion', async () => {
      ...
      await browser.wait(EC.presenceOf(element(by.cssContainingText('.m-message', 'new messages'))), THROTTLE_DURATION);
      expect(element(by.cssContainingText('.m-message', 'new messages for the discussion')).isPresent()).toEqual(true);
      expect(element(by.cssContainingText('.m-message', 'new other messages')).isPresent()).toEqual(true);
      expect(element(by.css('.notification')).isPresent()).toEqual(false);
    });
