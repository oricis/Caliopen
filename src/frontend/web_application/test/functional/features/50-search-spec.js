...
describe('search', () => {
  ...
  it('search for Foobar', () => {
    ...
    browser.get('./')
      ...
      .then(() => browser.wait(EC.presenceOf($('.l-search-results')), 1000))
      .then(() => expect(element(by.cssContainingText('.m-navbar-item', __('tab_label_foobar'))).isPresent()).toEqual(true))
      .then(() => expect(element(by.cssContainingText('.l-search-results__panel', __('msg_label_foobar'))).isPresent()).toEqual(true))
      .then(() => expect(element(by.cssContainingText('.l-search-results__panel', __('contact_label_foobar'))).isPresent()).toEqual(true))
      .then(() => expect(element.all(by.css('.s-message-item')).count()).toEqual(2))
