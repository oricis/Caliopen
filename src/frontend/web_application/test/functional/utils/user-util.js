...
module.exports = {
  signin: (login, password) => {
    ...
    return browser.get('/auth/signin')
      .then(() => element(by.css('input[name=username]')).sendKeys(loginKeys))
      .then(() => element(by.css('input[name=password]')).sendKeys(passwordKeys))
      .then(() => element(by.cssContainingText('button[type=submit]', 'I\'m in a safe place')).click())
      .then(() => browser.wait(EC.not(EC.urlContains('auth/signin')), 5 * 1000))
    ;
  },
